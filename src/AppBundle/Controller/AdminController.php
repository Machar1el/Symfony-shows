...
class AdminController extends Controller
{
    ...
    public function deleteSeasonAction($id)
    {
        ...
        $em = $this->get('doctrine')->getManager();
        $repository = $em->getRepository('AppBundle:Season');

        if ($season = $repository->find($id)) {
            $show_id = $season->getShow()->getId();

            $em->remove($season);       // Génération des requêtes SQL

            $episodes = $season->getEpisodes();

            foreach ($episodes as $episode) {
                $em->remove($episode);  // Génération des requêtes SQL
            }

            $em->flush();               // Envoi des requêtes SQL
